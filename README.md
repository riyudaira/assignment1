...
2026年3月16日：Stripe決済機能の不具合および利便性を修正

- **SSL証明書エラー（errno 77）の解消**
  - Dockerコンテナ（PHP-FPM）内に ca-certificates を設定しました。
  - 念の為、Stripe SDKに対して証明書パス（/etc/ssl/certs/ca-certificates.crt）を明示的に指定する処理を PurchaseController に追加しました。

- **動作確認用シーディングデータの追加**
  - Stripe決済が利用できない場合でも、決済後の機能（取引チャット、評価機能など）を確認できるよう、初期状態で「購入済み商品」が存在するように ItemsTableSeeder を更新しました。テストユーザー1、テストユーザー2がそれぞれ5品ずつ出品し、互いの商品を購入しています。

- **チャット画面のUI改善**
  - 取引商品が多数並んだ際にサイドバーのみを独立してスクロールできるようcssを修正しました。
